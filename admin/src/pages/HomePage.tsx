...
function cascadeToggleField(
	collections: CollectionConfig[],
	changedUid: string,
	fieldName: string,
	willBeEnabled: boolean,
	allContentTypes: CollectionConfig[],
): CollectionConfig[] {
	const changedCollection = collections.find((c) => c.uid === changedUid);
	const changedField = changedCollection?.fields.find(
		(f) => f.name === fieldName,
	);

	if (
		!changedField ||
		changedField.type !== 'relation' ||
		!changedField.relationTarget
	) {
		return collections;
	}

	const targetUid = changedField.relationTarget;

	if (!willBeEnabled) {
		return collections
			.map((col) => {
				if (col.uid !== targetUid) return col;
				const updatedFields = col.fields.map((f) =>
					f.type === 'relation' && f.relationTarget === changedUid
						? { ...f, enabled: false }
						: f,
				);
			...
			.filter((col) => {
				if (col.uid !== targetUid) return true;
				return col.fields.some((f) => f.enabled);
			});
	}

	const targetAlreadyActive = collections.some((c) => c.uid === targetUid);
	...
	if (targetAlreadyActive) {
		...
		const fullTarget = allContentTypes.find((c) => c.uid === targetUid);
		if (!fullTarget) return collections;

		const freshTarget: CollectionConfig = {
			...JSON.parse(JSON.stringify(fullTarget)),
			fields: fullTarget.fields.map((f) => ({
				...f,
				enabled: f.type === 'relation' && f.relationTarget === changedUid,
			})),
		};
		return [...collections, freshTarget];
	}
}

function cascadeRemoveCollection(
	collections: CollectionConfig[],
	removedUid: string,
): CollectionConfig[] {
	return collections
		.filter((col) => col.uid !== removedUid)
		.map((col) => {
			const updatedFields = col.fields.map((f) => {
				if (f.type === 'relation' && f.relationTarget === removedUid) {
		...
		.filter((col) => col.fields.some((f) => f.enabled));
}
				...
				(ct: any) => ({
					...
					fields: ct.attributes
						...
						.map((attr: any) => ({
							name: attr.name,
							type: attr.type,
							relationTarget: attr.relationTarget ?? null,
					...
					>
						<ResponseTemplates
							...
							onToggleField={(uid, fName) => {
								...
								const col = current.find((c) => c.uid === uid);
								const field = col?.fields.find((f) => f.name === fName);
								const willBeEnabled = !field?.enabled;

								const toggled = current.map((c) =>
									c.uid !== uid
										? c
										: {
												...c,
								...
								const cascaded = cascadeToggleField(
									toggled,
									uid,
									fName,
									willBeEnabled,
								...
								setValue('activeCollections', cascaded, { shouldDirty: true });
								setCollectionError('');
							}}
							onToggleAll={(uid, val) => {
								const current = getValues('activeCollections');
								...
								if (!val) {
									...
									const relationFields =
										col?.fields.filter(
											(f) => f.type === 'relation' && f.relationTarget,
										) || [];
									for (const rf of relationFields) {
										updated = cascadeToggleField(
											updated,
											uid,
											rf.name,
											false,
							...
							onRemoveCollection={(uid) => {
								const current = getValues('activeCollections');
								const cascaded = cascadeRemoveCollection(current, uid);
								setValue('activeCollections', cascaded, { shouldDirty: true });
							}}

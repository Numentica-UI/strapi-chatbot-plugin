...
A: ${answer}`;
    const result = await strapi.plugin("faq-ai-bot").service("embed").generateEmbedding(textToEmbed);
    if (result) {
...
const config$1 = ({ strapi }) => ({
  ...
  async update(ctx) {
    ...
    if (settings.openaiKey && existingSettings?.openaiKey !== settings.openaiKey) {
      await pluginStore.set({
        key: "token_usage",
...
async function rephraseQuestion(strapi, history, question, usage) {
...
async function searchFAQ(question, strapi, usage) {
  const openai = await getOpenAI$1(strapi);
...
async function simplePlanner(strapi, question, activeCollections, instructions, usage) {
  const openai = await getOpenAI$1(strapi);
  const response = await openai.chat.completions.create({
    model: "gpt-4o-mini",
    temperature: 0,
...
async function realtimeInterpreterAI(strapi, question, realtimeData, usage) {
...
async function finalAggregator(strapi, ctx, question, faq, realtimeMeta, realtimeText, contactLink, instructions, cardStyles, usage) {
...
const ask = ({ strapi }) => ({
  ...
  async getUsage(ctx) {
    ...
    const inputCost = usage.promptTokens / 1e6 * 0.15;
    const outputCost = usage.completionTokens / 1e6 * 0.6;
    const embeddingCost = usage.embeddingTokens / 1e6 * 0.02;
    ctx.body = {
